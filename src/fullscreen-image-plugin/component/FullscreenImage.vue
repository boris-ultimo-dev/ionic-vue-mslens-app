...
const isNextDisabled = computed(
  () => panoramaCurrentIndex.value === props.imageUrl.length - 1
);
      ...
      <template v-if="Array.isArray(imageUrl)">
        <transition-group name="list" tag="div" class="image-container">
          <button
            ...
            :class="['icon', isPreviousDisabled && 'icon--disabled']"
          ...
          <template
            v-for="(currImage, index) in imageUrl"
            :key="currImage + index"
          >
            <img
          ...
          <button
            ...
            :class="['icon', isNextDisabled && 'icon--disabled']"
            @click="handleNext"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
